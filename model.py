...
class baseline_DQN:
    ...
    def _build_net(self):
        ...
        b_initializer = tf.constant_initializer(0.1) #y=xw+b
        n_l1 = 20  # config of layers

        # ------------------ build evaluate_net ------------------
        self.s = tf.placeholder(tf.float32, [None, self.n_features], name='s')  # input # 11 type+vm
    ...
    def store_transition(self, s, a, r, s_): # 用字典存储更易维护和修改
    ...
    def learn(self):
        ...
        for i in range(self.batch_size):
            q_real_batch.append(minibatch[i][2] + self.gamma * np.max(q_next_batch[i]))# r+gamma* maxaQ
        # train eval network
        ...
        # increasing epsilon
        if self.epsilon < self.epsilon_max: # 训练一次 随机选择的概率下降 直到下降到最低值
    ...
    def print_experience_buffer(self): # 输出此刻存储的经验四元组
        for i, experience in enumerate(self.replay_buffer):
